...
test_GenomicRanges_findKNearest_all_k <- function()
{
    ## zero-length edge cases
    x <- IntegerList()
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 0), integer(length(x)))
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 1), integer(length(x)))
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 2), integer(length(x)))
    ...
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 0), integer(length(x)))
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 1), c(0L, 1L))
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 2), c(0L, 1L))

    ## essential functionality
    x <- IntegerList(1L)
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 0), integer(length(x)))
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 1), 1L)
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 2), 1L)

    x <- IntegerList(1L, 1:2)
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 0), integer(length(x)))
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 1), c(1L, 1L))
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 2), 1:2)
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 3), 1:2)

    ## ties
    x <- IntegerList(1:2, c(1L, 1:2)) # ties at start
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 0), integer(length(x)))
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 1), 1:2)
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 2), c(2L, 2L))
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 3), c(2L, 3L))
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 4), c(2L, 3L))

    x <- IntegerList(1:2, c(1:2, 2L)) # ties at end
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 0), integer(length(x)))
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 1), c(1L, 1L))
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 2), c(2L, 3L))
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 3), c(2L, 3L))
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 4), c(2L, 3L))
    ...
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 1), 1:2)
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 2), c(2L, 2L))
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 3), c(2L, 4L))
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 4), c(2L, 4L))
    checkIdentical(GenomicRanges:::.findKNearest_all_k(x, 5), c(2L, 4L))
}

test_GenomicRanges_findKNearest <- function()
{
    ## single argument
    g1 <- GRanges()
    checkIdentical(IntegerList(), findKNearest(g1))

    ## empty ranges
    g1 <- GRanges()
    g2 <- GRanges()
    checkIdentical(IntegerList(), findKNearest(g1, g2))
    ...
    checkIdentical(nearest(g), unlist(findKNearest(g)))

    ## Same strand non-overlapping, x and subject
    g1 <- GRanges(c("chr1:1-5:+", "chr1:7-11:+"))
    g2 <- GRanges(c("chr1:2-3:+", "chr1:10-12:+"))
    checkIdentical(nearest(g1, g2), unlist(findKNearest(g1, g2)))

    ## Same strand non-overlapping, some have no neighbors
    g1 <- GRanges(c("chr1:1-5:+", "chr1:11-15:+", "chr2:11-15:+"))
    g2 <- GRanges("chr1:8-9:+")
    checkIdentical(IntegerList(1, 1, NA), findKNearest(g1, g2))

    ## Same strand, ties
    x <- GRanges("chr1:15")
    subject<- GRanges(c("chr1:10", "chr1:20", "chr1:30"))

    current <- findKNearest(x, subject)
    checkIdentical(IntegerList(2), current) # arbitrary choice
    current <- findKNearest(x, subject, k = 2)
    checkIdentical(IntegerList(c(2, 1)), current)
    current <- findKNearest(x, subject, k = 3)
    checkIdentical(IntegerList(c(2, 1, 3)), current)

    current <- findKNearest(x, subject, k = 1, select = "all")
    checkIdentical(IntegerList(c(2, 1)), current)
    current <- findKNearest(x, subject, k = 2, select = "all")
    checkIdentical(IntegerList(c(2, 1)), current)
    current <- findKNearest(x, subject, k = 3, select = "all")
    checkIdentical(IntegerList(c(2, 1, 3)), current)

    subject<- GRanges(c("chr1:10", "chr1:20", "chr1:17", "chr1:30"))
    current <- findKNearest(x, subject, k = 2, select = "all")
    checkIdentical(IntegerList(c(3, 2, 1)), current)
    curent <- findKNearest(x, subject, k = 3, select = "all")
    checkIdentical(IntegerList(c(3, 2, 1)), current)
    current<- findKNearest(x, subject, k = 4, select = "all")
    ...
    nearest_all <- findKNearest(g3, g4, select="all")
    ...
    near <- findKNearest(g1, g2, ignore.strand = TRUE)
    checkIdentical(length(near), 2L)
    checkIdentical(length(near[[1]]), 1L)
    checkIdentical(length(near[[2]]), 1L)
    checkIdentical(near[[1]], 1L)
    ...
    near2 <- findKNearest(g3, g4, ignore.strand = TRUE)
...
#    checkIdentical(length(near)[[2]], length(near2)[[2]])
    ...
    ## select == "all"
    ## No ties
    #checkIdentical(findKNearest(g1, g2), findKNearest(g1, g2, select = "all"))
    ## With ties
    r <- IRanges(c(14, 1, 6, 10, 10, 14), c(16, 4, 8, 12, 12, 16))
    g <- GRanges("chr1", r, "+")
    target <- findKNearest(g, select="all")
