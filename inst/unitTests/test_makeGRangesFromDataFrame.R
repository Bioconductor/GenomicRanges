...
test_find_GRanges_cols <- function()
{
    ...
    target <- c(seqnames=3L, start=2L, end=1L, strand=NA_integer_)
    current <- find_GRanges_cols(df_colnames)
    checkIdentical(target, current)

    df_colnames <- c("strand", "STOP", "START", "chromosome_name")
    target <- c(seqnames=4L, start=3L, end=2L, strand=1L)
    current <- find_GRanges_cols(df_colnames)
    checkIdentical(target, current)

    df_colnames <- c("Seqnames", "strand", "start", "end")
    target <- c(seqnames=1L, start=3L, end=4L, strand=2L)
    current <- find_GRanges_cols(df_colnames)
    checkIdentical(target, current)
    ...
    target <- c(seqnames=5L, start=3L, end=4L, strand=2L)
    current <- find_GRanges_cols(df_colnames)
    checkIdentical(target, current)
    ...
    target <- c(seqnames=4L, start=5L, end=1L, strand=NA_integer_)
    current <- find_GRanges_cols(df_colnames)
    checkIdentical(target, current)
    ...
    target <- c(seqnames=4L, start=2L, end=1L, strand=NA_integer_)
    current <- find_GRanges_cols(df_colnames)
    checkIdentical(target, current)
    ...
    checkException(find_GRanges_cols(df_colnames), silent=TRUE)
    target <- c(seqnames=1L, start=2L, end=4L, strand=NA_integer_)
    ...
    checkException(find_GRanges_cols(df_colnames), silent=TRUE)
    target <- c(seqnames=4L, start=2L, end=3L, strand=NA_integer_)
