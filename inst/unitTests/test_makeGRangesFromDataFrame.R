...
test_find_core_GRanges_cols <- function()
{
    ...
    current <- find_core_GRanges_cols(df_colnames)
    ...
    target <- c(seqnames=1L, start=3L, end=4L,
                width=NA_integer_, strand=NA_integer_)
    current <- find_core_GRanges_cols(df_colnames, ignore.strand=TRUE)
    checkIdentical(target, current)
    ...
    checkException(find_core_GRanges_cols(df_colnames), silent=TRUE)
    ...
    target <- c(seqnames=1L, start=2L, end=4L,
                width=NA_integer_, strand=NA_integer_)
    current <- find_core_GRanges_cols(df_colnames, end.field="stop")
    checkIdentical(target, current)
    checkException(find_core_GRanges_cols(df_colnames, end.field=4),
                   silent=TRUE)
    ...
    target <- c(seqnames=4L, start=2L, end=3L,
                width=NA_integer_, strand=NA_integer_)
    current <- find_core_GRanges_cols(df_colnames, seqnames.field="chr")
    checkIdentical(target, current)
