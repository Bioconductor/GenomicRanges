...
regions are distributed. With this level of control, reads are not double counted. 
...
interest (\Robject{query}) and reads (\Robject{subj}), using short 
...
One might also create \Robject{query} with \Rfunction{makeTranscriptDbFromUCSC},
...
\Robject{subj} might normally come from BAM files (e.g.,
...
The \Robject{query} and \Robject{subj} objects contain the genes and reads
...
The \Robject{query} contains 11 genes where
...
Our \Robject{subj} contains a total of nine reads,
...
overlaps of a specific \Rfunarg{type} and reads (subj) that
hit a single region (query) are recorded. Reads that hit
multiple regions are resolved with one of
...
is assigned to the feature the disjoint region
originated from.
...
fragment has a value of 1/3 to contribute to the region 
