...
<<ov14>>=
ov14 <- findOverlaps(gal14, dm3tx, ignore.strand=TRUE)
@
...
<<ovenc14of>>=
ovenc14o <- encodeOverlaps(grl14, dm3tx, hits=ov14)
ovenc14f <- encodeOverlaps(grl14f, dm3tx, hits=ov14)
@
...
<<ovenc14_again>>=
ovenc14_again <- encodeOverlaps(grl14, dm3tx, hits=ov14, flip.query.if.wrong.strand=TRUE)
...
head(gal14)
table(gal14_ncompat)
mean(gal14_ncompat >= 1)
@

71.45\% of the alignments in \Rcode{gal14} are ``compatible''
with at least 1 transcript in \Rcode{dm3tx}.

Number of ``compatible'' overlaps per transcript in \Rcode{dm3tx}:

<<dm3tx_ncompat14>>=
dm3tx_ncompat14 <- tabulate(subjectHits(ov14)[ov14_is_compat], nbins=length(dm3tx))
names(dm3tx_ncompat14) <- names(dm3tx)
tail(table(dm3tx_ncompat14))
mean(dm3tx_ncompat14 >= 1)
@

Only 1\% of the transcripts in \Rcode{dm3tx} are ``compatible''
with at least 1 alignment in \Rcode{gal14}.
...
<<ov14_qstart>>=
ov14_qstart <- extractQueryStartInTranscript(grl14, dm3tx,
                                             hits=ov14, ovenc=ovenc14)
head(subset(ov14_qstart, ov14_is_compat))
@

\Rcode{ov14\_qstart} is a data frame with 1 row per overlap and 3 columns:
\begin{enumerate}
...
\Rcode{grl14}. We can reuse \Rfunction{extractTranscriptsFromGenome} for
this:

<<qseq14>>=
qseq14 <- extractTranscriptsFromGenome(Dmelanogaster, grl14)
@

Now we compare the read and transcript sequences for each ``compatible''
overlap. Because the read sequences (in \Rcode{qseq14}) and the transcript
...
<<ov14_qseq_vs_ov14_txseq>>=
ov14_qseq <- qseq14[queryHits(ov14)]
ov14_qseq[flippedQuery(ovenc14)] <- reverseComplement(ov14_qseq[flippedQuery(ovenc14)])
ov14_txseq <- txseq[subjectHits(ov14)]
stopifnot(all(
    ov14_qseq[ov14_is_compat] == narrow(ov14_txseq[ov14_is_compat],
                                        start=ov14_qstart$startInTranscript[ov14_is_compat],
                                        width=width(ov14_qseq)[ov14_is_compat])
))
@
...
gal14 <- readGappedAlignments(untreated1_chr4(), use.names=TRUE, param=param1)
bamseq14 <- elementMetadata(gal14)$seq
names(bamseq14) <- names(gal14)
head(bamseq14)
@
...
<<seq14>>=
seq14 <- bamseq14
is_on_minus <- as.logical(strand(gal14) == "-")
seq14[is_on_minus] <- reverseComplement(seq14[is_on_minus])
head(seq14)
@
...
<<no_indels_in_gal14>>=
colSums(cigarOpTable(cigar(gal14)))
@

So we should only see mismatches (typically a small number) between the
sequences in \Rcode{seq14} and those in \Rcode{qseq14}, but no indels,
...
the first 500 sequences in \Rcode{gal14}:

<<seq14_vs_qseq14>>=
gal14_nedit500 <- sapply(1:500, function(i) neditAt(seq14[[i]], qseq14[[i]]))
table(gal14_nedit500)
@
...
\Rcode{gal14\_nedit500}:

<<gal14_nedit>>=
gal14_nedit <- elementMetadata(gal14)$NM
stopifnot(all(gal14_nedit[1:500] == gal14_nedit500))
@
...
mean(gal14_nalmostcompat >= 1)
@

Only 0.2\% of the alignments in \Rcode{gal14} are ``almost compatible''
with at least 1 transcript in \Rcode{dm3tx}.

Number of ``almost compatible'' overlaps per transcript in \Rcode{dm3tx}:

<<dm3tx_nalmostcompat14>>=
dm3tx_nalmostcompat14 <- tabulate(subjectHits(ov14)[ov14_is_almostcompat], nbins=length(dm3tx))
names(dm3tx_nalmostcompat14) <- names(dm3tx)
table(dm3tx_nalmostcompat14)
mean(dm3tx_nalmostcompat14 >= 1)
@
...
<<ov14_qstart>>=
head(subset(ov14_qstart, ov14_is_almostcompat))
@
...
f <- names(dm3tx)[subjectHits(ov14)[is_nov_splice_jct]]
...
<<ov34>>=
ov34 <- findOverlaps(galp34, dm3tx, ignore.strand=TRUE)
@
...
ovenc34 <- encodeOverlaps(grl34, dm3tx, hits=ov34, flip.query.if.wrong.strand=TRUE)
ovenc34
@
...
mean(galp34_ncompat >= 1)
@

66.8\% of the alignment pairs in \Rcode{galp34} are ``compatible''
with at least 1 transcript in \Rcode{dm3tx}.

Number of ``compatible'' overlaps per transcript in \Rcode{dm3tx}:

<<dm3tx_ncompat34>>=
dm3tx_ncompat34 <- tabulate(subjectHits(ov34)[ov34_is_compat], nbins=length(dm3tx))
names(dm3tx_ncompat34) <- names(dm3tx)
tail(table(dm3tx_ncompat34))
mean(dm3tx_ncompat34 >= 1)
@

Only 1\% of the transcripts in \Rcode{dm3tx} are ``compatible''
with at least 1 alignment pair in \Rcode{galp34}.
...
<<ov34_Lqstart>>=
ov34_Lqstart <- extractQueryStartInTranscript(grl34, dm3tx,
                                              hits=ov34, ovenc=ovenc34)
head(subset(ov34_Lqstart, ov34_is_compat))
@
...
<<ov34_Rqstart>>=
ov34_Rqstart <- extractQueryStartInTranscript(grl34, dm3tx,
                                              hits=ov34, ovenc=ovenc34,
                                              for.query.right.end=TRUE)
head(subset(ov34_Rqstart, ov34_is_compat))
@

In order to perform the same kind of sanity check that we did for the
single-end reads, we need to split \Rcode{grl34} in 2 \Rclass{GRangesList}
objects of the same length as \Rcode{grl34}, one containing the ranges of
...
<<split_grl34>>=
grl34_first <- grglist(first(galp34), order.as.in.query=TRUE)
grl34_last <- grglist(last(galp34, invert.strand=TRUE), order.as.in.query=TRUE)
@

Then we extract the sequences from the reference genome based on the ranges
in those 2 \Rclass{GRangesList} objects:

<<qseq34_first_and_last>>=
qseq34_first <- extractTranscriptsFromGenome(Dmelanogaster, grl34_first)
qseq34_last <- extractTranscriptsFromGenome(Dmelanogaster, grl34_last)
@

and we assign 2 sequences to each overlap, one for each end of the read:

<<ov34_Lqseq_and_Rqseq>>=
ov34_Lqseq <- qseq34_first[queryHits(ov34)]
ov34_Rqseq <- qseq34_last[queryHits(ov34)]
@
...
need to swap them between \Rcode{ov34\_Lqseq} and \Rcode{ov34\_Rqseq}:

<<ov34_Lqseq_and_Rqseq>>=
flip_idx <- which(flippedQuery(ovenc34))
tmp <- ov34_Lqseq[flip_idx]
ov34_Lqseq[flip_idx] <- reverseComplement(ov34_Rqseq[flip_idx])
ov34_Rqseq[flip_idx] <- reverseComplement(tmp)
@

We assign the transcript sequence to each overlap:

<<ov34_txseq>>=
ov34_txseq <- txseq[subjectHits(ov34)]
@
...
<<ov34_Lqseq_vs_ov34_txseq>>=
stopifnot(all(
    ov34_Lqseq[ov34_is_compat] == narrow(ov34_txseq[ov34_is_compat],
                                         start=ov34_Lqstart$startInTranscript[ov34_is_compat],
                                         width=width(ov34_Lqseq)[ov34_is_compat])
))
@

and the ``right ends'':

<<ov34_Rqseq_vs_ov34_txseq>>=
stopifnot(all(
    ov34_Rqseq[ov34_is_compat] == narrow(ov34_txseq[ov34_is_compat],
                                         start=ov34_Rqstart$startInTranscript[ov34_is_compat],
                                         width=width(ov34_Rqseq)[ov34_is_compat])
))
@
...
our current \Rcode{galp34}, we need to do this again but now we do it at
load time which is equivalent to doing it afterward (i.e. not only do we
have the guarantee to end up with the same elements in \Rcode{galp34}, but
also to have them in the same order):

<<bamseq34_first_and_last>>=
...
galp34 <- readGappedAlignmentPairs(untreated3_chr4(), use.names=TRUE, param=param2)

bamseq34_first <- elementMetadata(first(galp34))$seq
names(bamseq34_first) <- names(galp34)
head(bamseq34_first)

bamseq34_last <- elementMetadata(last(galp34))$seq
names(bamseq34_last) <- names(galp34)
head(bamseq34_last)
@
...
<<seq34_first_and_last>>=
seq34_first <- bamseq34_first
is_on_minus <- as.logical(strand(first(galp34)) == "-")
seq34_first[is_on_minus] <- reverseComplement(seq34_first[is_on_minus])

seq34_last <- bamseq34_last
is_on_minus <- as.logical(strand(last(galp34)) == "-")
seq34_last[is_on_minus] <- reverseComplement(seq34_last[is_on_minus])
@
...
<<no_indels_in_galp34>>=
colSums(cigarOpTable(cigar(first(galp34))))
colSums(cigarOpTable(cigar(last(galp34))))
@

So we should only see mismatches (typically a small number) between the
{\it original} sequences (in \Rcode{seq34\_first} and \Rcode{seq34\_last})
and those in \Rcode{qseq34\_first} and \Rcode{qseq34\_last}, but no indels.
We do this comparison for the first 500 sequences in \Rcode{galp34}:

<<seq34_vs_qseq34>>=
galp34_first_nedit500 <- sapply(1:500, function(i)
    neditAt(seq34_first[[i]], qseq34_first[[i]])
)
table(galp34_first_nedit500)

galp34_last_nedit500 <- sapply(1:500, function(i)
    neditAt(reverseComplement(seq34_last[[i]]), qseq34_last[[i]])
)
table(galp34_last_nedit500)
@
...
<<galp34_nedit_vs_NM>>=
galp34_first_nedit <- elementMetadata(first(galp34))$NM
stopifnot(all(galp34_first_nedit[1:500] == galp34_first_nedit500))

galp34_last_nedit <- elementMetadata(last(galp34))$NM
stopifnot(all(galp34_last_nedit[1:500] == galp34_last_nedit500))
@
...
table(galp34_first_nedit + galp34_last_nedit)
table(galp34_first_nedit)
table(galp34_last_nedit)
@
