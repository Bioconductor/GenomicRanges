...
In this document, we will also need to access the {\it original query
sequences}. To load them, we reload the BAM file but now we explicitely
request the SEQ field by using \Rcode{what="seq"} in our call to
\Rfunction{ScanBamParam}. We also load the NM tag (we will need it too),
which is a predefined tag described in the SAM Spec as the ``Edit distance
to the reference, including ambiguous bases but excluding clipping''
(note that tags are optional fields i.e. not all BAM files have them):
...
Sequences with the same {\it query name} correspond to the same {\it original
query} and therefore should be the same:

<<same_name_implies_same_seq_in_U1seq>>=
stopifnot(all(
    U1seq[match(names(U1seq), names(U1seq))] == U1seq
))
@

Finally, since indels were not allowed/supported during the alignment process,
the NM tag (edit distance) is actually equal to the number of mismatches.
The maximum observed number of mismatches tells us how many mismatches per
read were tolerated by the aligner:
...
input files, typically 1 FASTQ file for the ``first ends'' and 1 FASTQ file
for the ``last ends''), we shouldn't expect the names of \Rcode{U3galp} to be
unique:
...
<<same_name_implies_same_seq_in_U3seq>>=
stopifnot(all(
    U3seq_first[match(names(U3seq_first), names(U3seq_first))] == U3seq_first
))
stopifnot(all(
    U3seq_last[match(names(U3seq_last), names(U3seq_last))] == U3seq_last
))
@

Finally, the following table tells us how many mismatches per read were
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Projecting the reads on the transcripts}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Projecting the single-end reads on the transcripts}
...
Furthermore, we'll be able to show that, for ``compatible'' overlaps,
the edit distance reported in the NM tag of the BAM file is actually
the edit distance between the read and the transcript.
...
Instead of using the {\it original query sequences} for this comparison,
we'll use the {\it reference query sequences} i.e. the sequences obtained
by extracting the portions of the reference genome corresponding to the
ranges in \Rcode{U1grl}. We can reuse \Rfunction{extractTranscriptsFromGenome}
for this:
...
overlap. Because the read sequences (in \Rcode{U1rqseq}) and the transcript
...
Thanks to the above result, comparing the {\it original query sequences}
to the transcript sequences is equivalent to comparing them to the
{\it reference query sequences}. And because the aligned reads have no indels,
we should only see mismatches (typically a small number) during that
comparison so we don't need to call \Rfunction{neditAt} with
\Rcode{with.indels=TRUE}.
Let's do this comparison for the first 500 sequences in \Rcode{U1gal}:

<<U1seq_vs_U1rqseq>>=
U1gal_nedit500 <- sapply(1:500, function(i) neditAt(U1seq[[i]], U1rqseq[[i]]))
...
Assuming that this is true for all the sequences in \Rcode{U1seq},
then we would be able to conclude that the edit distance reported in
the NM tag is actually the edit distance between the read and transcript
sequences of a ``compatible'' overlap.
...
Like with single-end reads, having this information allows us to compare the
read and transcript nucleotide sequences for each ``compatible'' overlap.
Here too, instead of using the {\it original query sequences} for this
comparison, we'll use the {\it reference query sequences} i.e. the sequences
obtained by extracting the portions of the reference genome corresponding to
the ranges in \Rcode{U3grl}. However, because our reads are paired-end, we
need to split \Rcode{U3grl} in 2 \Rclass{GRangesList} objects of the same
length as \Rcode{U3grl}, one containing the ranges of the ``first ends''
(aka ``first segments'' in BAM jargon), and one containing the ranges of
the ``last ends'' (aka ``last segments'' in BAM jargon). Here is an efficient
way to do this:
...
For each ``compatible'' overlap, we expect the ``left and right sequences''
of the read to be {\it exact} substrings of the transcript sequence:

Let's check the ``left sequences'':

<<U3ov_Lrqseq_vs_U3ov_txseq>>=
stopifnot(all(
...
and the ``right sequences'':

<<U3ov_Rrqseq_vs_U3ov_txseq>>=
stopifnot(all(
...
Thanks to the above result, and like with our single-end reads, comparing the
{\it original query sequences} to the transcript sequences is equivalent to
comparing them to the {\it reference query sequences}. And because the aligned
reads have no indels, we should only see mismatches (typically a small number)
during that comparison.
Let's do this comparison for the first 500 sequences in \Rcode{U3galp}:

<<U3seq_vs_U3rqseq>>=
U3galp_first_nedit500 <- sapply(1:500, function(i)
    neditAt(U3seq_first[[i]], U3rqseq_first[[i]])
)
table(U3galp_first_nedit500)

U3galp_last_nedit500 <- sapply(1:500, function(i)
    neditAt(reverseComplement(U3seq_last[[i]]), U3rqseq_last[[i]])
)
...
Again, assuming that this is true for all the sequences in \Rcode{U3galp},
then we would be able to conclude that the edit distance reported in
the NM tag is actually the edit distance between the read and transcript
sequences of a ``compatible'' overlap.
