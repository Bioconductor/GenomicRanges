...
stopifnot(all(U3galp_oqseq1[U3galp_dup2unq] == U3galp_oqseq1))
stopifnot(all(U3galp_oqseq2[U3galp_dup2unq] == U3galp_oqseq2))
@
...
Number of ``compatible'' transcripts per alignment in \Rcode{U1gal}:
...
per {\it original query}, we also count the number of ``compatible''
...
Number of ``compatible'' reads per transcript:
...
Number of ``compatible'' transcripts per alignment pair in \Rcode{U3galp}:
...
``compatible'' transcripts per read:
...
70.1\% of the reads are ``compatible'' with at least 1 transcript in
\Rcode{dm3tx}.
...
Only 0.75\% of the transcripts in \Rcode{dm3tx} are ``compatible'' with at
least 50 reads.
...
Number of ``string-based compatible'' transcripts per read:
...
Number of ``string-based compatible'' reads per transcript:
