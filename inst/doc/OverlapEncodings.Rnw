...
<<U1gal_qnames>>=
U1gal_qnames <- factor(names(U1gal), levels=unique(names(U1gal)))
@
...
<<U3galp_qnames>>=
U3galp_qnames <- factor(names(U3galp), levels=unique(names(U3galp)))
@
...
U1gal_seq <- elementMetadata(U1gal)$seq
names(U1gal_seq) <- names(U1gal)
head(U1gal_seq)
@
...
stopifnot(all(U1gal_oqseq[U1gal_dup2unq] == U1gal_oqseq))
@
...
<<U3bamseq>>=
U3galp_first_seq <- elementMetadata(first(U3galp))$seq
U3galp_last_seq <- elementMetadata(last(U3galp))$seq
names(U3galp_first_seq) <- names(U3galp_last_seq) <- names(U3galp)
head(U3galp_first_seq)
head(U3galp_last_seq)
@
...
U3galp_first_oqseq <- U3galp_first_seq
U3galp_first_is_on_minus <- as.logical(strand(first(U3galp)) == "-")
U3galp_first_oqseq[U3galp_first_is_on_minus] <- reverseComplement(U3galp_first_oqseq[U3galp_first_is_on_minus])

U3galp_last_oqseq <- U3galp_last_seq
U3galp_last_is_on_minus <- as.logical(strand(last(U3galp)) == "-")
U3galp_last_oqseq[U3galp_last_is_on_minus] <- reverseComplement(U3galp_last_oqseq[U3galp_last_is_on_minus])
@
...
stopifnot(all(U3galp_first_oqseq[U3galp_dup2unq] == U3galp_first_oqseq))
stopifnot(all(U3galp_last_oqseq[U3galp_dup2unq] == U3galp_last_oqseq))
@
...
U3galp_first_rqseq <- extractTranscriptsFromGenome(Dmelanogaster, U3grl_first)
U3galp_last_rqseq <- extractTranscriptsFromGenome(Dmelanogaster, U3grl_last)
@
...
\Rcode{U3galp\_first\_oqseq} and in
\Rcode{reverseComplement(U3galp\_last\_oqseq)}:

<<U3-oqseq-vs-U3-rqseq>>=
U3galp_first_nedit500 <- sapply(1:500, function(i)
    neditAt(U3galp_first_oqseq[[i]], U3galp_first_rqseq[[i]])
)
table(U3galp_first_nedit500)

U3galp_last_nedit500 <- sapply(1:500, function(i)
    neditAt(reverseComplement(U3galp_last_oqseq[[i]]), U3galp_last_rqseq[[i]])
)
table(U3galp_last_nedit500)
@

Yes, the first 500 sequences in \Rcode{U3galp\_first\_oqseq} and in
\Rcode{reverseComplement(U3galp\_last\_oqseq)} are ``close'' to the first 500
sequences in \Rcode{U3galp\_first\_rqseq} and in \Rcode{U3galp\_last\_rqseq},
respectively.
...
count the number of overlaps per query or the number of overlaps per subject:
...
<<U1uqnames_ncompat>>=
U1uqnames_ncompat <- nhitPerQuery(remapHits(U1compatov, query.map=U1gal_qnames))
...
<<dm3tx_U1ncompat>>=
dm3tx_U1ncompat <- nhitPerSubject(U1compatov)
names(dm3tx_U1ncompat) <- names(dm3tx)
head(sort(dm3tx_U1ncompat, decreasing=TRUE), n=10)  # top 10 transcripts
mean(dm3tx_U1ncompat >= 50)
@

Only 0.835\% of the transcripts in \Rcode{dm3tx} are ``compatible''
...
``compatible'' overlaps per unique {\it query template name}:

<<U3uqnames_ncompat>>=
U3uqnames_ncompat <- nhitPerQuery(remapHits(U3compatov, query.map=U3galp_qnames))
...
<<dm3tx_U3ncompat>>=
dm3tx_U3ncompat <- nhitPerSubject(U3compatov)
names(dm3tx_U3ncompat) <- names(dm3tx)
head(sort(dm3tx_U3ncompat, decreasing=TRUE), n=10)  # top 10 transcripts
mean(dm3tx_U3ncompat >= 50)
@

Only 0.73\% of the transcripts in \Rcode{dm3tx} are ``compatible''
with at least 50 alignment pairs in \Rcode{U3galp}.
...
U3ov_Lrqseq <- U3galp_first_rqseq[queryHits(U3ov)]
U3ov_Rrqseq <- U3galp_last_rqseq[queryHits(U3ov)]
@
...
The single-end reads are in \Rcode{U1gal\_oqseq}, the transcriptome is in
\Rcode{dm3tx\_seq}.
...
on chr4, we don't need to align \Rcode{U1gal\_oqseq} to the full transcriptome,
...
which.txseq <- match(dm3chr4txids, names(dm3tx_seq))
@
...
then find the ``hits'' for the ``flipped {\it original queries}'', and
finally merge all the ``hits'' (note that the 3 commands take about
10-12 minutes to complete on a modern laptop):

<<U1hits, eval=FALSE>>=
U1hits <- findSequenceHits(U1gal_oqseq, dm3tx_seq,
                           which.txseq=which.txseq, max.mismatch=6)
U1hitsf <- findSequenceHits(reverseComplement(U1gal_oqseq), dm3tx_seq,
                            which.txseq=which.txseq, max.mismatch=6)
...
Number of ``string-based compatible'' overlaps per alignment in
\Rcode{U1gal}:

<<U1gal_nsbcompat>>=
U1gal_nsbcompat <- nhitPerQuery(U1hits)
...
76.44\% of the alignments in \Rcode{U1gal} are ``string-based compatible''
with at least 1 transcript in \Rcode{dm3tx}.

Number of ``string-based compatible'' overlaps per unique {\it query name}:

<<U1uqnames_nsbcompat>>=
U1uqnames_nsbcompat <- nhitPerQuery(remapHits(U1hits, query.map=U1gal_qnames))
table(U1uqnames_nsbcompat)
mean(U1uqnames_nsbcompat >= 1)
@

77.8\% of the {\it original queries} are ``string-based compatible'' with
at least 1 transcript in \Rcode{dm3tx}.

Number of ``compatible'' overlaps per transcript in \Rcode{dm3tx}:

<<dm3tx_U1nsbcompat>>=
dm3tx_U1nsbcompat <- nhitPerSubject(U1hits)
names(dm3tx_U1nsbcompat) <- names(dm3tx)
head(sort(dm3tx_U1nsbcompat, decreasing=TRUE), n=10)  # top 10 transcripts
mean(dm3tx_U1nsbcompat >= 50)
@

Only 0.839\% of the transcripts in \Rcode{dm3tx} are ``string-based
compatible'' with at least 50 alignments in \Rcode{U1gal}.
...
But now we want to look at those ``string-based compatible'' overlaps
that are not ``encoding-based compatible''. We divide them in 2 groups:
\begin{itemize}
  \item Group A: Overlaps in \Rcode{U1ov} (i.e. found by \Rcode{findOverlaps})
        that are ``string-based compatible'' but not ``encoding-based
        compatible''.
  \item Group B: Overlaps not even in \Rcode{U1ov} (i.e. not found by
        \Rcode{findOverlaps}).
\end{itemize}

<<U1hitsAB>>=
U1hitsAB <- setdiff(U1hits, U1ov[U1ov_is_compat])
U1hitsB <- intersect(U1hitsAB, U1ov)
U1hitsA <- setdiff(U1hitsAB, U1hitsB)
@
...
head(sort(dm3tx_U1nalmostcompat, decreasing=TRUE), n=10)  # top 10 transcripts
mean(dm3tx_U1nalmostcompat >= 50)
@

Only 0.004\% of the transcripts in \Rcode{dm3tx} are ``almost compatible''
with at least 50 alignments in \Rcode{U1gal}.
