...
Now we compare the read and transcript sequences of the first 500
``compatible'' overlaps only. Because the read sequences (in \Rcode{rseq14})
and the transcript sequences (in \Rcode{txseq}) were both extracted from
the reference genome, we expect the formers to be {\it exact} substrings
of the latters:
...
stopifnot(all(
    sapply(which(ov14_is_compat)[1:500],
           function(i) {
               ...
               subject_subseq <- subseq(subject_seq,
                                        start=ov14_qstart$startInTranscript[i],
                                        width=length(query_seq))
               query_seq == subject_subseq
           })
))
@
...
Is seems that the aligner was able to tolerate up to 6 mismatches:
