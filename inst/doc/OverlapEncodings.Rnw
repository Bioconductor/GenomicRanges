...
Encodings \Rcode{"1:i:"} (395278 occurences in \Rcode{U1ovenc}),
\Rcode{"2:jm:af:"} (65169 occurences in \Rcode{U1ovenc}),
and \Rcode{"3:jmm:agm:aaf:"} (342 occurences in \Rcode{U1ovenc}),
...
\Rcode{U3ovenc} contains 13 unique paired-end encodings ``compatible'' with
...
\Rcode{U3ovenc}), \Rcode{"2{-}{-}1:jm{-}{-}m:af{-}{-}i:"} (2562 occurences
in \Rcode{U3ovenc}), \Rcode{"1{-}{-}2:i{-}{-}jm:a{-}{-}af:"} (2023 occurences
in \Rcode{U3ovenc}), \Rcode{"1{-}{-}1:i{-}{-}m:a{-}{-}i:"} (353 occurences
in \Rcode{U3ovenc}), and \Rcode{"2{-}{-}2:jm{-}{-}mm:af{-}{-}jm:aa{-}{-}af:"}
(122 occurences in \Rcode{U3ovenc}), correspond to the following paired-end
overlaps:
...
table(U3ov00_is_comp)  # 94068 "compatible" paired-end overlaps
@
...
Encodings \Rcode{"2:jm:am:af:"} (721 occurences in \Rcode{U1ovenc}),
\Rcode{"2:jm:am:am:af:"} (112 occurences in \Rcode{U1ovenc}),
...
table(U1ov00_is_acomp)  # 860 "almost compatible" overlaps
@

Finally, let's extract the ``almost compatible'' overlaps from \Rcode{U1ov00}:
...
Number of ``almost compatible'' transcripts for each alignment in \Rcode{U1gal}:
...
\Rcode{U3ovenc} contains 5 unique paired-end encodings ``almost compatible''
with the splicing of the transcript:

<<U3-unique-almost-compatible-encodings>>=
sort(U3ovenc_table[isCompatibleWithSkippedExons(U3_unique_encodings)])
@

Paired-end encodings \Rcode{"2{-}{-}1:jm{-}{-}m:am{-}{-}m:af{-}{-}i:"}
(54 occurences in \Rcode{U3ovenc}),
\Rcode{"1{-}{-}2:i{-}{-}jm:a{-}{-}am:a{-}{-}af:"} (36 occurences in
\Rcode{U3ovenc}), and
\Rcode{"2{-}{-}2:jm{-}{-}mm:am{-}{-}mm:af{-}{-}jm:aa{-}{-}af:"}
(9 occurences in \Rcode{U3ovenc}), correspond to the following paired-end
overlaps:
...
<<U3ov00_is_acomp>>=
U3ov00_is_acomp <- isCompatibleWithSkippedExons(U3ovenc)
table(U3ov00_is_acomp)  # 104 "almost compatible" paired-end overlaps
@
...
<<U3acompov00>>=
U3acompov00 <- U3ov00[U3ov00_is_acomp]
@
...
<<U3galp_nacomptx>>=
U3galp_nacomptx <- nhitPerQuery(U3acompov00)
elementMetadata(U3galp)$nacomptx <- U3galp_nacomptx
head(U3galp)
table(U3galp_nacomptx)
mean(U3galp_nacomptx >= 1)
@

Only 0.18\% of the alignment pairs in \Rcode{U3galp} are ``almost compatible''
with at least 1 transcript in \Rcode{exbytx}.

Number of ``almost compatible'' alignment pairs for each transcript:

<<exbytx_nU3acompov00>>=
exbytx_nU3acompov00 <- nhitPerSubject(U3acompov00)
names(exbytx_nU3acompov00) <- names(exbytx)
table(exbytx_nU3acompov00)
mean(exbytx_nU3acompov00 >= 50)
@
...
<<U3ov00_Lqstart-and-U3ov00_Rqstart>>=
head(subset(U3ov00_Lqstart, U3ov00_is_acomp))
head(subset(U3ov00_Rqstart, U3ov00_is_acomp))
@
...
First we extract the index of those alignments ({\it nsh} here means
``{\bf n}ovel {\bf s}plicing {\bf h}int''):

<<U1gal_is_nsh>>=
U1gal_is_nsh <- U1gal_nacomptx != 0L & U1gal_ncomptx == 0L
head(which(U1gal_is_nsh))
@

We make this an index into \Rcode{U1ov00}:

<<U1ov00_is_nsh>>=
U1ov00_is_nsh <- queryHits(U1ov00) %in% which(U1gal_is_nsh)
@

We intersect with \Rcode{U1ov00\_is\_acomp} and then subset \Rcode{U1ov00}
to keep only the overlaps that suggest novel splicing:

<<narrow-U1ov00_is_nsh>>=
U1ov00_is_nsh <- U1ov00_is_nsh & U1ov00_is_acomp
U1nshov00 <- U1ov00[U1ov00_is_nsh]
@

For each overlap in \Rcode{U1nshov00}, we extract the ranks of the skipped
exons (we use a list for this as there might be more than 1 skipped
exon per overlap):

<<U1nshov00_skippedexranks>>=
U1nshov00_skippedexranks <- extractSkippedExonRanks(U1ovenc)[U1ov00_is_nsh]
names(U1nshov00_skippedexranks) <- queryHits(U1nshov00)
table(elementLengths(U1nshov00_skippedexranks))
@

Finally, we split \Rcode{U1nshov00\_skippedexranks} by transcript names:

<<exbytx_U1skippedexranks>>=
f <- factor(names(exbytx)[subjectHits(U1nshov00)], levels=names(exbytx))
exbytx_U1skippedexranks <- split(U1nshov00_skippedexranks, f)
@

\Rcode{exbytx\_U1skippedexranks} is a named list of named lists of integer
...
<<names-of-exbytx_U1skippedexranks>>=
head(names(exbytx_U1skippedexranks))  # transcript names
@

Transcript FBtr0089124 receives 7 hits. All of them skip exons 9 and 10:

<<FBtr0089124-skipped-exons>>=
exbytx_U1skippedexranks$FBtr0089124
@
...
exbytx_U1skippedexranks$FBtr0089147
@

A few words about the interpretation of \Rcode{exbytx\_U1skippedexranks}:
Because of how we've conducted this analysis, the aligments reported in
\Rcode{exbytx\_U1skippedexranks} are guaranteed to not have any ``compatible''
