...
<<same_name_implies_same_seq_in_U1oqseq>>=
stopifnot(all(
    U1oqseq[match(names(U1oqseq), names(U1oqseq))] == U1oqseq
))
@
...
<<same_name_implies_same_seq_in_U3oqseq>>=
stopifnot(all(
    U3oqseq_first[match(names(U3oqseq_first), names(U3oqseq_first))] == U3oqseq_first
))
stopifnot(all(
    U3oqseq_last[match(names(U3oqseq_last), names(U3oqseq_last))] == U3oqseq_last
))
@
...
be an {\it exact} substring of the transcript sequence in \Rcode{txseq}:

<<U1ov_rqseq_vs_U1ov_txseq>>=
...
Aligning the {\it original queries} to the reference genome is not the
most efficient nor accurate way to count the number of ``compatible''
overlaps per {\it original query}. Supporting junction reads (i.e. reads
that align with at least 1 gap) introduces a significant computational cost
during the alignment process. Then, as we've seen in the previous sections,
each alignment produced by the aligner needs to be broken into a set of
ranges (based on its CIGAR) and those ranges compared to the ranges of
the exons grouped by transcript.

A more straightforward and accurate approach is to align the {\it original
queries} directly to the transcriptome.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Align the single-end reads to the transcriptome}

Since indels were not allowed/supported for the alignment of the reads to
...
``compatible'' overlaps between reads and transcripts, we don't keep track
of the details of alignments between the reads and transcripts. Those
simplifications allow us to write the following function that we will use for
finding the ``compatible'' overlaps:

<<findCompatibleOverlaps>>=
### A wrapper to vwhichPDict() that supports ambiguity codes in 'oqseq'
### and 'txseq', and treats them as such.
findCompatibleOverlaps <- function(oqseq, txseq, max.mismatch=0)
{
