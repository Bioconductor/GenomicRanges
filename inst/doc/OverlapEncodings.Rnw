...
transcript nucleotide sequences for each overlap. Let's first extract
the transcript sequences from the \Rpackage{BSgenome.Dmelanogaster.UCSC.dm3}
data package with the \Rfunction{extractTranscriptsFromGenome} function from
...
Now we compare the sequences associated with the first 500 ``compatible''
overlaps only. Because the read sequences (in \Rcode{rseq14}) and the
transcript sequences (in \Rcode{txseq}) were both extracted from the
reference genome, we expect of course exact matches between the sequences.
...
To load the ``real'' read sequences, we reload the BAM file and explicitely
request the SEQ field. We also load the NM tag:
...
Because the BAM format imposes that the read sequence is
``reverse-complemented'' when the read is aligned to the minus strand,
we need to ``reverse-complement'' it again to get the original sequence:
...
It seems that indels were not allowed/supported when the reads were
aligned (no I or D CIGAR operations):

<<cigarOpTable>>=
colSums(cigarOpTable(cigar(gal14)))
@

so we expect a small number of mismatches between the sequences in
\Rcode{seq14} and those in \Rcode{rseq14}, but no indels:
...
Note that the edit distance is actually stored in the NM tag:

<<gal14_nedit>>=
gal14_nedit <- elementMetadata(gal14)$NM
stopifnot(all(gal14_nedit[1:500] == gal14_nedit500))
table(gal14_nedit)  # up to 6 mismatches!
@
