...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Position of the read within the transcript for a ``compatible''
            overlap}
...
transcript nucleotide sequences for each overlap. Let's start by using
the \Rfunction{extractTranscriptsFromGenome} function from the
\Rpackage{GenomicFeatures} package to extract the transcript sequences
from the \Rpackage{BSgenome.Dmelanogaster.UCSC.dm3} data package:
...
overlap. Because the read sequences (in \Rcode{rseq14}) and the transcript
...
ov14_qseq <- rseq14[queryHits(ov14)]
...
<<bamseq14>>=
param <- ScanBamParam(flag=flag0, what="seq", tag="NM")
gal14 <- readGappedAlignments(untreated1_chr4(), use.names=TRUE, param=param)
...
The following table indicates that indels were not allowed/supported when
the reads were aligned (no I or D CIGAR operations):
...
sequences in \Rcode{seq14} and those in \Rcode{rseq14}, but no indels,
...
<<seq14_vs_rseq14>>=
gal14_nedit500 <- sapply(1:500, function(i) neditAt(seq14[[i]], rseq14[[i]]))
...
Finally, the maximum observed number of mismatches tells us how many mismatches
per read the aligner tolerated during the alignment process:
