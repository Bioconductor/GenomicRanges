...
retrieve the dm3 transcripts and their exons from UCSC, and then extract
the exons grouped by transcript in a \Rclass{GRangesList} object.
...
We use the \Rfunction{makeTranscriptDbFromUCSC} function defined in the
\Rpackage{GenomicFeatures} package to retrieve the dm3 transcripts and
their exons from UCSC:
...
length(dm3tx)  # nb of transcripts
@
...
from our \Rclass{TranscriptDb} object \Rcode{dm3\_ensGene\_txdb}, and
then we construct a named factor that represents the mapping:

<<dm3tx2gene>>=
tx <- transcripts(dm3_ensGene_txdb, columns=c("tx_name", "gene_id"))
head(tx)
df <- elementMetadata(tx)
dm3tx2gene <- as.character(df$gene_id)
dm3tx2gene <- factor(dm3tx2gene, levels=unique(dm3tx2gene))
names(dm3tx2gene) <- df$tx_name
dm3tx2gene <- dm3tx2gene[names(dm3tx)]
head(dm3tx2gene)
nlevels(dm3tx2gene)  # nb of genes
@
...
<<U1ov00>>=
U1ov00 <- findOverlaps(U1gal, dm3tx, ignore.strand=TRUE)
@
...
<<dm3tx_nU1ov10>>=
dm3tx_nU1ov10 <- nhitPerSubject(U1ov10)
names(dm3tx_nU1ov10) <- names(dm3tx)
mean(dm3tx_nU1ov10 >= 50)
@
...
<<top-10-transcripts-based-on-dm3tx_nU1ov10>>=
head(sort(dm3tx_nU1ov10, decreasing=TRUE), n=10) 
@
...
<<U3ov00>>=
U3ov00 <- findOverlaps(U3galp, dm3tx, ignore.strand=TRUE)
@
...
<<dm3tx_nU3ov10>>=
dm3tx_nU3ov10 <- nhitPerSubject(U3ov10)
names(dm3tx_nU3ov10) <- names(dm3tx)
mean(dm3tx_nU3ov10 >= 50)
@
...
<<top-10-transcripts-based-on-dm3tx_nU3ov10>>=
head(sort(dm3tx_nU3ov10, decreasing=TRUE), n=10)
@
...
<<U1ovencAB>>=
U1ovencA <- encodeOverlaps(U1grl, dm3tx, hits=U1ov00)
U1ovencB <- encodeOverlaps(U1grlf, dm3tx, hits=U1ov00)
@
...
<<U1ovenc_again>>=
U1ovenc_again <- encodeOverlaps(U1grl, dm3tx, hits=U1ov00, flip.query.if.wrong.strand=TRUE)
...
U3ovenc <- encodeOverlaps(U3grl, dm3tx, hits=U3ov00, flip.query.if.wrong.strand=TRUE)
U3ovenc
@
...
U1compov00_again <- findCompatibleOverlaps(U1gal, dm3tx)
...
U1gal_ncomptx_again <- countCompatibleOverlaps(U1gal, dm3tx)
...
<<dm3tx_nU1compov10>>=
dm3tx_nU1compov10 <- nhitPerSubject(U1compov10)
names(dm3tx_nU1compov10) <- names(dm3tx)
mean(dm3tx_nU1compov10 >= 50)
@
...
<<top-10-transcripts-based-on-dm3tx_nU1compov10>>=
head(sort(dm3tx_nU1compov10, decreasing=TRUE), n=10)
@
...
U3compov00_again <- findCompatibleOverlaps(U3galp, dm3tx)
...
U3galp_ncomptx_again <- countCompatibleOverlaps(U3galp, dm3tx)
...
<<dm3tx_nU3compov10>>=
dm3tx_nU3compov10 <- nhitPerSubject(U3compov10)
names(dm3tx_nU3compov10) <- names(dm3tx)
mean(dm3tx_nU3compov10 >= 50)
@
...
<<top-10-transcripts-based-on-dm3tx_nU3compov10>>=
head(sort(dm3tx_nU3compov10, decreasing=TRUE), n=10)
@
...
<<U1ov00_qstart>>=
U1ov00_qstart <- extractQueryStartInTranscript(U1grl, dm3tx,
...
U1ov00_txseq <- dm3tx_seq[subjectHits(U1ov00)]
stopifnot(all(
...
<<U3ov00_Lqstart>>=
U3ov00_Lqstart <- extractQueryStartInTranscript(U3grl, dm3tx,
...
<<U3ov00_Rqstart>>=
U3ov00_Rqstart <- extractQueryStartInTranscript(U3grl, dm3tx,
                                              hits=U3ov00, ovenc=U3ovenc,
...
<<U3ov00_txseq>>=
U3ov00_txseq <- dm3tx_seq[subjectHits(U3ov00)]
@
...
U1sbcomphitsA <- findSequenceHits(U1oqseq, dm3tx_seq,
                                    which.txseq=which.txseq, max.mismatch=6)
U1sbcomphitsB <- findSequenceHits(reverseComplement(U1oqseq), dm3tx_seq,
                                    which.txseq=which.txseq, max.mismatch=6)
...
<<dm3tx_nU1sbcomphits>>=
dm3tx_nU1sbcomphits <- nhitPerSubject(U1sbcomphits)
names(dm3tx_nU1sbcomphits) <- names(dm3tx)
mean(dm3tx_nU1sbcomphits >= 50)
@
...
<<top-10-transcripts-based-on-dm3tx_nU1sbcomphits>>=
head(sort(dm3tx_nU1sbcomphits, decreasing=TRUE), n=10)
@
...
length(setdiff(U1compov10, U1sbcomphits))
@
...
<<dm3tx_nU1acompov00>>=
dm3tx_nU1acompov00 <- nhitPerSubject(U1acompov00)
names(dm3tx_nU1acompov00) <- names(dm3tx)
table(dm3tx_nU1acompov00)
mean(dm3tx_nU1acompov00 >= 50)
@
...
f <- names(dm3tx)[subjectHits(U1ov00)[is_nov_splice_jct]]
