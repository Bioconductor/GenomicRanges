...
is a general rule, not only when working with {\it overlap encodings}).
...
\Rcode{U1ov} is a \Rclass{Hits} object that contains 1 element per
...
the overlaps in a given \Rclass{Hits} object (e.g. \Rcode{U1ov}), i.e. to
...
and \Rcode{dm3tx} for computing \Rcode{U1gal\_ntx}:

<<U1gal_ntx_again, eval=FALSE>>=
U1gal_ntx_again <- countOverlaps(U1gal, dm3tx, ignore.strand=TRUE)
...
<<U1uqnames_ntx>>=
U1hits <- remapHits(U1ov, query.map=U1gal_qnames)
U1uqnames_ntx <- nhitPerQuery(U1hits)
...
<<dm3tx_U1nhit>>=
dm3tx_U1nhit <- nhitPerSubject(U1hits)
names(dm3tx_U1nhit) <- names(dm3tx)
mean(dm3tx_U1nhit >= 50)
@

Only 0.869\% of the transcripts in \Rcode{dm3tx} have an overlap with
at least 50 reads.

Top 10 transcripts:

<<top-10-transcripts-based-on-dm3tx_U1nhit>>=
head(sort(dm3tx_U1nhit, decreasing=TRUE), n=10) 
@
...
Like \Rcode{U1ov}, \Rcode{U3ov} is a \Rclass{Hits} object. Its length gives
...
and \Rcode{dm3tx} for computing \Rcode{U3galp\_ntx}:

<<U3galp_ntx_again, eval=FALSE>>=
U3galp_ntx_again <- countOverlaps(U3galp, dm3tx, ignore.strand=TRUE)
...
<<U3uqnames_ntx>>=
U3hits <- remapHits(U3ov, query.map=U3galp_qnames)
U3uqnames_ntx <- nhitPerQuery(U3hits)
...
<<dm3tx_U3nhit>>=
dm3tx_U3nhit <- nhitPerSubject(U3hits)
names(dm3tx_U3nhit) <- names(dm3tx)
mean(dm3tx_U3nhit >= 50)
@

Only 0.756\% of the transcripts in \Rcode{dm3tx} have an overlap with
at least 50 templates.

Top 10 transcripts:

<<top-10-transcripts-based-on-dm3tx_U3nhit>>=
head(sort(dm3tx_U3nhit, decreasing=TRUE), n=10)
@
...
<<U1ovencAB>>=
U1ovencA <- encodeOverlaps(U1grl, dm3tx, hits=U1ov)
U1ovencB <- encodeOverlaps(U1grlf, dm3tx, hits=U1ov)
@
...
<<U1ovenc_table>>=
U1_unique_encodings <- levels(encoding(U1ovenc))
...
Let's encode the overlaps in \Rcode{U3ov}:
...
<<U3ovenc_table>>=
U3_unique_encodings <- levels(encoding(U3ovenc))
...
Finally, let's extract the ``compatible'' overlaps from \Rcode{U1ov}:

<<U1compatov>>=
U1compatov <- U1ov[U1ov_is_compat]
@
...
<<U1compatov_again, eval=FALSE>>=
U1compatov_again <- findCompatibleOverlaps(U1gal, dm3tx)
stopifnot(identical(U1compatov_again, U1compatov))
@
...
<<U1gal_ncompat>>=
U1gal_ncompat <- nhitPerQuery(U1compatov)
...
can be used directly on \Rcode{U1gal} and \Rcode{dm3tx} for computing
\Rcode{U1gal\_ncompat}:

<<U1gal_ncompat_again, eval=FALSE>>=
U1gal_ncompat_again <- countCompatibleOverlaps(U1gal, dm3tx)
stopifnot(identical(U1gal_ncompat_again, U1gal_ncompat))
@

Number of ``compatible'' transcripts for each read:

<<U1uqnames_ncompat>>=
U1compathits <- remapHits(U1compatov, query.map=U1gal_qnames)
U1uqnames_ncompat <- nhitPerQuery(U1compathits)
names(U1uqnames_ncompat) <- U1uqnames
...
<<dm3tx_U1ncompat>>=
dm3tx_U1ncompat <- nhitPerSubject(U1compathits)
names(dm3tx_U1ncompat) <- names(dm3tx)
mean(dm3tx_U1ncompat >= 50)
@

Only 0.860\% of the transcripts in \Rcode{dm3tx} are ``compatible''
with at least 50 reads.

Top 10 transcripts:

<<top-10-transcripts-based-on-dm3tx_U1ncompat>>=
head(sort(dm3tx_U1ncompat, decreasing=TRUE), n=10)
@
...
Finally, let's extract the ``compatible'' overlaps from \Rcode{U3ov}:

<<U3compatov>>=
U3compatov <- U3ov[U3ov_is_compat]
@
...
<<U3compatov_again, eval=FALSE>>=
U3compatov_again <- findCompatibleOverlaps(U3galp, dm3tx)
stopifnot(identical(U3compatov_again, U3compatov))
@
...
<<U3galp_ncompat>>=
U3galp_ncompat <- nhitPerQuery(U3compatov)
...
Note that high-level convenience wrapper \Rfunction{countCompatibleOverlaps}
can be used directly on \Rcode{U3galp} and \Rcode{dm3tx} for computing
\Rcode{U3galp\_ncompat}:

<<U3galp_ncompat_again, eval=FALSE>>=
U3galp_ncompat_again <- countCompatibleOverlaps(U3galp, dm3tx)
stopifnot(identical(U3galp_ncompat_again, U3galp_ncompat))
@

Number of ``compatible'' transcripts for each template:

<<U3uqnames_ncompat>>=
U3compathits <- remapHits(U3compatov, query.map=U3galp_qnames)
U3uqnames_ncompat <- nhitPerQuery(U3compathits)
names(U3uqnames_ncompat) <- U3uqnames
...
<<dm3tx_U3ncompat>>=
dm3tx_U3ncompat <- nhitPerSubject(U3compathits)
names(dm3tx_U3ncompat) <- names(dm3tx)
mean(dm3tx_U3ncompat >= 50)
@

Only 0.752\% of the transcripts in \Rcode{dm3tx} are ``compatible'' with at
least 50 templates.

Top 10 transcripts:

<<top-10-transcripts-based-on-dm3tx_U3ncompat>>=
head(sort(dm3tx_U3ncompat, decreasing=TRUE), n=10)
@
...
<<dm3tx_seq>>=
dm3tx_seq <- extractTranscriptsFromGenome(Dmelanogaster, dm3tx)
@

For each ``compatible'' overlap, the read sequence in \Rcode{U1gal\_rqseq} must
be an {\it exact} substring of the transcript sequence in \Rcode{dm3tx\_seq}:

<<U1ov_rqseq-vs-U1ov_txseq>>=
U1ov_rqseq <- U1gal_rqseq[queryHits(U1ov)]
U1ov_rqseq[flippedQuery(U1ovenc)] <- reverseComplement(U1ov_rqseq[flippedQuery(U1ovenc)])
U1ov_txseq <- dm3tx_seq[subjectHits(U1ov)]
stopifnot(all(
...
U3ov_Lrqseq <- U3galp_rqseq1[queryHits(U3ov)]
U3ov_Rrqseq <- U3galp_rqseq2[queryHits(U3ov)]
@
...
<<U3ov_txseq>>=
U3ov_txseq <- dm3tx_seq[subjectHits(U3ov)]
@
...
but only to the subset of \Rcode{dm3tx\_seq} made of the transcripts located
on chr4.
...
Let's compute the index of the transcripts in \Rcode{dm3tx\_seq} located on
chr4 (\Rfunction{findSequenceHits} will restrict the search to those
transcripts):

<<which.txseq, eval=FALSE>>=
dm3chr4tx <- transcripts(dm3_ensGene_txdb, vals=list(tx_chrom="chr4"))
dm3chr4txnames <- elementMetadata(dm3chr4tx)$tx_name
which.txseq <- match(dm3chr4txnames, names(dm3tx_seq))
@
...
<<U1sbcompathits, eval=FALSE>>=
U1sbcompathitsA <- findSequenceHits(U1oqseq, dm3tx_seq,
                                    which.txseq=which.txseq, max.mismatch=6)
U1sbcompathitsB <- findSequenceHits(reverseComplement(U1oqseq), dm3tx_seq,
                                    which.txseq=which.txseq, max.mismatch=6)
U1sbcompathits <- union(U1sbcompathitsA, U1sbcompathitsB)
@
<<LOAD_U1sbcompathits, echo=FALSE, results=hide>>=
U1sbcompathitsA <- .loadPrecomputed("U1sbcompathitsA")
U1sbcompathitsB <- .loadPrecomputed("U1sbcompathitsB")
U1sbcompathits <- union(U1sbcompathitsA, U1sbcompathitsB)
@
...
<<U1uqnames_nsbcompat>>=
U1uqnames_nsbcompat <- nhitPerQuery(U1sbcompathits)
names(U1uqnames_nsbcompat) <- U1uqnames
table(U1uqnames_nsbcompat)
mean(U1uqnames_nsbcompat >= 1)
@

77.7\% of the reads are ``string-based compatible'' with at least 1
transcript in \Rcode{dm3tx}.

Number of ``string-based compatible'' reads for each transcript:

<<dm3tx_U1nsbcompat>>=
dm3tx_U1nsbcompat <- nhitPerSubject(U1sbcompathits)
names(dm3tx_U1nsbcompat) <- names(dm3tx)
mean(dm3tx_U1nsbcompat >= 50)
@

Only 0.865\% of the transcripts in \Rcode{dm3tx} are ``string-based
compatible'' with at least 50 reads.

Top 10 transcripts:

<<top-10-transcripts-based-on-dm3tx_U1nsbcompat>>=
head(sort(dm3tx_U1nsbcompat, decreasing=TRUE), n=10)
@
...
length(setdiff(U1compathits, U1sbcompathits))
@

but the reverse is not true:

<<string-based-compatible-does-NOT-imply-encoding-based-compatible>>=
length(setdiff(U1sbcompathits, U1compathits))
@
...
%\Rcode{setdiff(U1sbcompathits, U1compathits)}. This is a perfect hit between
%read SRR031728.4692406 and transcript 18924:
%
%<<>>=
%matchPattern(U1oqseq[[6306]], dm3tx_seq[[18924]])
%U1gal_idx <- which(U1gal_qnames == "SRR031728.4692406")
%U1gal[U1gal_idx]
%U1gal_idx %in% queryHits(U1ov)
%U1gal[12636]
%which(queryHits(U1ov) == 12636)
%U1ov[305]
...
<<U1gal_nalmostcompat>>=
U1almostcompatov <- U1ov[U1ov_is_almostcompat]
U1gal_nalmostcompat <- nhitPerQuery(U1almostcompatov)
...
Number of ``almost compatible'' overlaps for each transcript in \Rcode{dm3tx}:

<<dm3tx_U1nalmostcompat>>=
dm3tx_U1nalmostcompat <- nhitPerSubject(U1almostcompatov)
names(dm3tx_U1nalmostcompat) <- names(dm3tx)
table(dm3tx_U1nalmostcompat)
mean(dm3tx_U1nalmostcompat >= 50)
@

Only 0.0087\% of the transcripts in \Rcode{dm3tx} are ``almost compatible''
