...
the nucleotide mapped to each read from the reference genome. This of
...
comparison for the first 500 sequences in \Rcode{U1seq} only:
...
Yes, the first 500 sequences in \Rcode{U1seq} are ``close'' to the first
...
Note that sequences with the same {\it query name} correspond to the same
{\it original query} and therefore should be the same. Let's do a quick
sanity check:
...
Let's do this comparison for the first 500 sequences in \Rcode{U3seq\_first}
and \Rcode{U3seq\_last}:
...
Yes, the first 500 sequences in \Rcode{U3seq\_first} and \Rcode{U3seq\_last}
...
to extract the transcript sequences from the dm3 reference genome:
