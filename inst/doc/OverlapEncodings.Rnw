...
comparison for the first 500 sequences in \Rcode{U1.GAL\_oqseq} only:
...
<<U1.GAL_is_nsh>>=
U1.GAL_is_nsh <- U1.GAL_nacomptx != 0L & U1.GAL_ncomptx == 0L
head(which(U1.GAL_is_nsh))
@

We make this an index into \Rcode{U1.OV00}:

<<U1.OV00_is_nsh>>=
U1.OV00_is_nsh <- queryHits(U1.OV00) %in% which(U1.GAL_is_nsh)
@
...
<<narrow-U1.OV00_is_nsh>>=
U1.OV00_is_nsh <- U1.OV00_is_nsh & U1.OV00_is_acomp
U1.nshOV00 <- U1.OV00[U1.OV00_is_nsh]
@

For each overlap in \Rcode{U1.nshOV00}, we extract the ranks of the skipped
exons (we use a list for this as there might be more than 1 skipped
exon per overlap):

<<U1.nshOV00_skippedexranks>>=
U1.nshOV00_skippedexranks <- extractSkippedExonRanks(U1.ovenc)[U1.OV00_is_nsh]
names(U1.nshOV00_skippedexranks) <- queryHits(U1.nshOV00)
table(elementLengths(U1.nshOV00_skippedexranks))
@

Finally, we split \Rcode{U1.nshOV00\_skippedexranks} by transcript names:

<<U1.exbytx_skippedexranks>>=
f <- factor(names(exbytx)[subjectHits(U1.nshOV00)], levels=names(exbytx))
U1.exbytx_skippedexranks <- split(U1.nshOV00_skippedexranks, f)
@

\Rcode{U1.exbytx\_skippedexranks} is a named list of named lists of integer
...
<<names-of-U1.exbytx_skippedexranks>>=
head(names(U1.exbytx_skippedexranks))  # transcript names
@

Transcript FBtr0089124 receives 7 hits. All of them skip exons 9 and 10:

<<FBtr0089124-skipped-exons>>=
U1.exbytx_skippedexranks$FBtr0089124
@
...
U1.exbytx_skippedexranks$FBtr0089147
@

A few words about the interpretation of \Rcode{U1.exbytx\_skippedexranks}:
Because of how we've conducted this analysis, the aligments reported in
\Rcode{U1.exbytx\_skippedexranks} are guaranteed to not have any ``compatible''
