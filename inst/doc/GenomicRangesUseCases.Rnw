...
subsetRep1 <-
  orcAlignsRep1[pData(alignData(orcAlignsRep1))[["nMismatchBestHit"]] <= 3L]
...
overlap with any of the known exons from \textit{Saccharomycess
cerevisiae}. It is interesting to note that in spite of having removed
all known exons, this object still contains about half of the original
...
Finally, we should then be able to go back to the BSGenome object and
retrieve the actual sequences.  In order to recover these sequences,
we are going to specify that this data came from chromosome 13, and we
will just retrieve the plus strand version of each (since the
experiment collapsed that data anyhow).
