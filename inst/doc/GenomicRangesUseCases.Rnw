...
\title{\Biocpkg{GenomicRanges} Use Cases}
\author{Bioconductor Team}
\date{Edited: October 2012; Compiled: \today}
...
This vignette demonstrates workflows for high throughput sequence analysis 
that involve the \Biocpkg{GenomicRanges} package. In the more detailed 
examples \Biocpkg{GenomicRanges} may be just one of several packages used 
to accomplish the task. Other workflows are more focused and highlight a 
single function or aspect of the package that is especially useful.

Resources available for learning more about the ranged-based functionality
in \Bioconductor{} are the IRanges, GenomicRanges and GenomicFeatures 
paper 
http://www.ploscompbiol.org/article/info%3Adoi%2F10.1371%2Fjournal.pcbi.1003118
 and the several vignettes available in the package. To view available 
vignettes use \Rcode{browseVignettes("GenomicRanges")}.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% 
%%% Count tables for RNA-Seq differential gene expression 
%%% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Count tables for RNA-Seq differential gene expression}
...
Counting with \Rfunction{summarizeOverlaps} does not retain the bam 
records in \R{}. Often the count summary is the important piece of
information and the individual record details are not needed. In
cases where closer inspection of the data is necessary there are
several functions are available for reading bam records into \R{} 
objects.
...
\subsubsection{single-end}
Single-end data can be read with the \Rfunction{readGAlignments} function.
...
Paired-end data are read with \Rfunction{readGAlignmentPairs}
or \Rfunction{readGAlignmentsList}. These functions use the same
mate paring algorithm but output different objects. Mate pairing
is performed when \Rcode{asMates=TRUE} in the BamFile, otherwise
records are treated as single-end. 

<<bam_3>>=
un3 <- untreated3_chr4()
gapairs <- readGAlignmentPairs(BamFile(un3, asMates=TRUE))
galist <- readGAlignmentsList(BamFile(un3, asMates=TRUE))
@

The \Robject{GAlignmentPairs} class holds only pairs; singletons, 
reads with unmapped mates, and secondary alignments are discarded. 
...
\subsubsection{Iterating with \Rcode{yieldSize}}
...
Iteration through a file requires that the bam be opened, repeatedly
...
<<bam_8>>=
open(bf)
ans <- integer(length(exbygene))
repeat {
    gal <- readGAlignments(bf)
    if (length(gal))
        ans <- ans + countOverlaps(exbygene, gal)
    else
        break
}
close(bf)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% 
%%%  Extracting DNA sequences of gene regions
%%% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Extract DNA sequences of gene regions}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% 
%%%  Create DNA consensus sequence for read group families 
%%% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Create DNA consensus sequences for read group `families'}

The motivation for this use-case comes from a study which explored the
...
Use sequenceLayer() to "lay" the query sequences and quality strings
on the reference.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% 
%%% Compute binned averages along a genome 
%%% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Compute binned averages along a genome}
