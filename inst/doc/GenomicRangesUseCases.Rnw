...
will probably want to do all the above steps for however many samples
we want to process, and then place each resulting vector into a
...
called \Robject{rpkm2}, then you could create your \Rpackage{DESeq}
object like this:

<<DESeqFrame, eval=FALSE>>=
deframe <- data.frame(rpkm, rpkm2)  
@
