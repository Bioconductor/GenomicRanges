...
\noindent 
and the projected end of the alignments on the plus strand.
Note: As of GenomicRanges >= 1.11.7, shift() no longer
trims out of bound ranges. Because of this, we have a little
more work to do for the positive strand ends.

<<EatonPossibleEnds>>=
posRangesRep1 <- rangesRep1[strand(rangesRep1) == "+"]
posEndsRep1 <- shift(posRangesRep1, 99)
posEndsRep1 <- trim(posRangesRep1)
...
from a yeast experiment and load it into a \Rclass{GAlignments}
...
library(Rsamtools)
aligns <- readGAlignments(testFile)
@
...
<<noOlap>>=
noOlap <- aligns[!overlapsAny(aligns, exonRanges)]
@
