...
Finally, in the list context of a \Rclass{GRanges} object, the
\Rmethod{elementMetadata} and \Rmethod{values} methods are both useful
for assigning to and accessing data again, but this time the data is
accessed at the list level.
...
\Rmethod{lapply} and \Rmethod{sapply} behave as you would expect, with
the \Rmethod{lapply} method returning a list and sapply returning a
more simplified output.
...
But as with \Rclass{IRanges} objects, there is also a multivariate
version of \Rmethod{sapply} defined for \Rclass{GRangesList} objects.
If you don't want the results simplified, then you can call the
\Rmethod{Map} method.
...
Finally, the \Rmethod{Reduce} method will behave as expected and will
allow the \Rclass{GRanges} objects to be collapsed across the whole of
the \Rclass{GRangesList} object.
...
For even more information on the \Rcode{GRanges} and
\Rcode{GRangesList} classes be sure to consult their respective man
pages.
