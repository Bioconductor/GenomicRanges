...
\author{Marc Carlson, Patrick Aboyoun, Herve Pages}
\date{\today}
...
Genomic Features that can be contained in an \Rclass{GRanges} object
can be any number of different things such as a set of binding sites,
transcripts, or exons.  As long as the features being represented are
all the same kind of genomic feature then it will makes sense to store
them all in a single \Rclass{GRanges} object.  In more concrete terms,
such features are defined by their position on one or several
chromosomes/contigs, their strandedness, and must always have a start
and end value relative to a specific genome build. These objects can
be created by using the \Rfunction{GRanges} constructor function. For
example,
...
\noindent creates a \Rclass{GRanges} object with 10 single interval
range features. The output of the \Rclass{GRanges} \Rmethod{show}
method provides some insight into how this information is stored
internally.  Each \Rclass{GRanges} object has both a left and right
hand region which are separated by \Rcode{|} symbols. The genomic
coordinates (seqnames, ranges, and strand) are located on the
left-hand side and the element metadata (annotation) columns are
located on the right. For this example, the metadata includes a
\Rcode{score} and \Rcode{GC}, but almost anything can be stored in the
metadata portion of a \Rclass{GRanges} object.

The components of the genomic coordinates within a \Rclass{GRanges}
object can be extracted using the \Rmethod{seqnames},
\Rmethod{ranges}, and \Rmethod{strand} accessor functions.
...
\Rclass{GRanges} objects can also be split apart.  But notice that the
object returned is no longer a \Rclass{GRanges} object but is actually
a \Rclass{GRangesList} object.  \Rclass{GRangesList} objects will be
discussed in detail in the next section.  For now, it is sufficient
to just see how split can separate a single \Rclass{GRanges} object
into what will basically act as a list of \Rclass{GRanges} objects.
...
by using the \Rmethod{c} and \Rmethod{append} methods.  Both
...
If you pass a second argument to the subset operator, then you can specify
which elementMetadata columns will be returned from the \Rclass{GRanges}
object.  So for example, I can request that only the 2nd column of the
elementMetadata portion of the \Rclass{GRanges} object be returned.

<<subset2>>=
gr[2:3,2]
@ 

And of course you can also assign into elements of the
\Rclass{GRanges} object as well with increasing degrees of specificity.
So for example we can replace the 2nd row of a \Rclass{GRanges} object
with the 1st row of \Robject{gr}.
...
Or you could assign only the metadata for score from the 3rd row to
replace only the score from the 2nd row etc.
<<assign2>>=
...
a specified width. You may notice that these methods respect the
boundaries imposed by beingon near end of the sequence (and also
...
different \Rclass{GRanges} objects.  These operations are extremely
useful, but note that they will drop element metadata from the return
values.  This is because there is no way for these methods to know
which metadata to retain.  Here are a some examples for how you can
calculate the \Rmethod{union}, the \Rmethod{intersect} and the
asymmetric difference (using \Rmethod{setdiff}).
...
both belong to and are defined by their respective transcripts.  Such
a feature makes much more sense when expressed as a compound object
such as a \Rclass{GRangesLists}.  Whenever genomic features consist of
...
it as a named list of \Rclass{GRanges} objects.  And, the names of
this list, are always the names of the grouping feature.  In the
example above, the groups of individual exon ranges are represented as
separate \Rclass{GRanges} objects which are further organized into a
...
\Rclass{GRanges} objects.  And, the \Rmethod{seqlengths} method will
...
\Rmethod{elementMetadata} and \Rmethod{values} methods are both useful
and perform a similar operation to what they did on a \Rclass{GRanges}
object.  But in this case, the data being accessed is at the list
level instead of the level of the individual \Rclass{GRanges} objects.

<<elementMetadataGRL>>=
elementMetadata(grl) <- c("Transcript A","Transcript B")
elementMetadata(grl)
values(grl)
@ 
...
%% Patrick: Why no worky?
% You can also append values together useing \Rmethod{append} or
% \Rmethod{c}, but be warned that the names from both will have to be
% unique for both \Rclass{GRangesList} objects in order for this to
% work.
...
And, these operations behave similarly as well.  The only difference
is that in this case, the operation will be applied to every element
of every \Rclass{GRanges} object in the \Rclass{GRangesList}, and the
result being another list object.
...
to again specify which of the elementMetadata columns you wish to select.
...
with the \Rmethod{lapply} method returning a list and sapply returning
...
things as \Rmethod{mapply} but without simplyfying the output.

% Check with Patrick to get a more interesting example method??
<<mapply>>=
grl2 <- shift(grl,10)
names(grl2) <- c("shiftTxA","shiftTxB")
...
Thus, we have the \Rmethod{endoapply} method which will return the
results as a modified \Rclass{GRangesList} object.
...
\Rclass{GRangesList} objects. To this end, the
\Rpackage{GenomicRanges} package provide a family of interval overlap
functions. The most general of these functions is
