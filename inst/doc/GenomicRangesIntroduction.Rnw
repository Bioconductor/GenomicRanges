...
are located on the left-hand side and the element metadata (annotation)
...
to specify which elementMetadata columns to extract from the
...
\Rmethod{elementMetadata} and \Rmethod{values} methods perform a similar
operations to what they do on a \Rclass{GRanges} object. However, this
metadata refers to information at the list level instead of the level of
the individual \Rclass{GRanges} objects.
...
subject as inputs and returns an \Rclass{RangesMatching} object containing
