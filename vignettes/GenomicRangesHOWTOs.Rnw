...
\subsection{Input data used in the HOWTOs}

In order to avoid repetition, input data that is used in more than one
{\it HOWTO} is described here:

\begin{itemize}
  \item The \Biocpkg{pasillaBamSubset} data package: contains both a BAM
        ...
        from the "Pasilla" experiment. See ?\Biocpkg{pasillaBamSubset}
...
\subsection{How to load a gene model from a GTF or GFF file}
