...
formally store genetic variants, with information on alleles and
read depths. The metadata columns, which were so useful during
prototyping, are too informal for use in our systematic
implementation: for the sake of data integrity, we need to ensure that
...
An orthogonal problem to data storage is the formalization of metadata
columns, and we solve this problem using the ``extra column slot''
mechanism. Whenever \Rclass{GenomicRanges} needs to operate on its
metadata columns, it also delegates to the internal
