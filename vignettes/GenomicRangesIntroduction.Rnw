...
\date{\today; updated 14 November, 2016}
...
The \Rmethod{elementNROWS} method returns a list of integers
corresponding to the result of calling \Rmethod{NROW} on each
...
Append lists using \Rmethod{append} or \Rmethod{c}.
