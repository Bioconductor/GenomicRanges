%\VignetteIndexEntry{An Introduction to the GenomicRanges Package}
...
\author{Marc Carlson \and Patrick Aboyoun \and Herv\'{e} Pag\`{e}s \and
  Martin Morgan}
\date{\today; updated 15 November, 2016}
...
(infrastructure) packages, and many other \software{Bioconductor} packages.
...
by using the \Rcode{c} and \Rcode{append} methods.
...
intervals. For example, the \Rcode{flank} method can be used to recover
...
Similar to \Rcode{flank}, there are also operations to
\Rcode{resize} and \Rcode{shift} our \Rclass{GRanges} object. The
\Rcode{shift} method will move the ranges by a specific number of
base pairs, and the \Rcode{resize} method will extend the ranges by
a specified width.
...
The \Rcode{reduce} will align the ranges and merge overlapping
...
The \Rcode{gaps} method will help you calculate the spaces between a
...
objects is the \Rcode{coverage} method. The \Rcode{coverage}
...
different \Rclass{GRanges} objects. Here are some examples for
how you can calculate the \Rcode{union}, the \Rcode{intersect}
and the asymmetric difference (using \Rcode{setdiff}).
...
<<example-GRangesList>>=
gr1 <-
  GRanges(seqnames = "chr2", ranges = IRanges(103, 106),
...
You can also use \Rcode{isEmpty} to test if a \Rclass{GRangesList}
...
\Rcode{mcols} method performs a similar operation to what it
...
The \Rcode{head}, \Rcode{tail}, \Rcode{rep}, \Rcode{rev},
and \Rcode{window} methods all behave as you would expect them
to for a list object. For example, the elements
referred to by \Rcode{window} are now list
...
\Rcode{apply} methods. These include \Rcode{lapply},
\Rcode{sapply}, \Rcode{mapply}, \Rcode{endoapply},
\Rcode{mendoapply}, \Rcode{Map}, and \Rcode{Reduce}.
...
This is achieved using the \Rcode{endoapply} method, which will return
...
And, there is also a multivariate version of the \Rcode{endoapply}
method in the form of the \Rcode{mendoapply} method.

<<mendoapply>>=
mendoapply(c,grl,grl2)
@

Finally, the \Rcode{Reduce} method will allow the \Rclass{GRanges}
